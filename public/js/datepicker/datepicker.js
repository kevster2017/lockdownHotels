var picker = {
  ...
  attach : function (opt) {
    ...
    if (opt.container) { 
      ...
      document.getElementById('checkInDateModalBody').appendChild(dp);
    }
    ...
    // (A6-P) POPUP DATE PICKER
      ...
      document.getElementById('staticBackdrop').appendChild(wrapper);
    }
  },
    ...
    for (var i=0; i<total; i++) {
      ...
      if (squares[i] == "B") { 
        ...
        // ALLOWED TO CHOOSE THIS DAY
          ...
          if (month <= today.getUTCMonth() && year == today.getUTCFullYear()) {
            cell.classList.add("picker-d-dd");
          }
          else if (squares[i][0] <= today.getUTCDate() && year == today.getUTCFullYear() && month == today.getUTCMonth()+1) { 
            cell.classList.add("picker-d-dd"); 
          }
          else {
  ...
  pick : function (el) {
    ...
    console.log(document.getElementById(parent.dataset.target).value);
    ...
    //document.getElementById('staticBackdrop').removeAttribute('aria-modal');
    //document.getElementById('staticBackdrop').removeAttribute('role');

    //const modalsBackdrops = document.getElementsByClassName('modal-backdrop');
    //document.body.lastChild.style.display = "none";
    
    /*for (var i=0; i < modalsBackdrops.length; i++) {
      console.log(modalsBackdrops[i]);
